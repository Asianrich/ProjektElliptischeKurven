...
\\
Bei der Programmierung gab es keine Schwierigkeit, da die Formeln schon angegeben war und der Abschlusskriterium.\\
Nur war das generieren der Primzahl relativ lang bei 8192-Bitlänge. Wenn man in 2er-Potenzen hochgehen würde, so hatte sich die Laufzeit Exponentiell erhöht.\\
Würde man jeden einzelnen Schritt debuggen, so hatte man bemerkt, das die Potenzrechnungen (bei hohen Zahlen) einige an Minuten gedauert haben.
Dies kann dadurch liegen, das die Bibliothek für BigInteger schlecht für diskrete Mathematik ist oder der Rechner konnte nicht so schnell rechnen.
Als man eine mögliche Zahl bekam, wurde dies an die oben genannte URL ausgetestet. Selbst die Seite hatte einige Minuten gebraucht, um die Zahl zu überprüfen.\\ 
\newpage
...
Alle Zahlen in $\mathbb{Z}$ geben nun die Restwerte raus.\\
Jetzt ist nun die Frage, ob man über diesen Körper Rechenoperationen durchführen können.
...
\begin{table}[!ht]
        ...
        \begin{tabular}{l}
            ...
            Wenn $a \in \mathbb{Z}_p$, dann ist (-a) die additive Inverse von a in $\mathbb{Z}_p$.\\
             Dies ist auch die einzige Lösung zur Gleichung : $a + x \equiv 0$ (mod p)\\
                        \bottomrule
        \end{tabular}
        \caption{Additive Inverse (Subtraktion) in der Endlichen Körper\cite{sec1}}
        \label{tab3}
    \end{table}
    ...
    \begin{table}[!ht]
        \centering
            \begin{tabular}{l}
                \toprule
                \textbf{Multiplikative Inverse (Division) in der Endlichen Körper}\\
                \midrule
                Wenn $a \in \mathbb{Z}_p, a \neq 0 $,  dann ist $a^{-1}$ die multiplikative Inverse von a in $\mathbb{Z}_p$.\\
                 ...
                 \bottomrule
            \end{tabular}
            \caption{Multiplikative Inverse (Division) in der Endlichen Körper\cite{sec1}}
            \label{tab3}
        \end{table}
\newpage
\subsection*{Beispiel}
...
Lediglich musste überprüft werden, ob die angegebene Zahl eine Primzahl ist oder nicht. Da aber der Fermat-Test schon fertig war,
konnte man einfach den Test aufrufen. 
\\
\newpage
