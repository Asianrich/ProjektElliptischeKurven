...
Bei einem k-Bit-Exponenten werden dann für die Potenzierung höchstens k Quadrate und k Multiplikationen benötigt \cite{lang}.
...
Die schnelle modulare Potenz kann jedoch auch in einfacher Weise realisiert werden \cite{lang}. Tatsächlich, kann \( x^{13}\) auch dargestellt werden, als 

\begin{align*}
    x^{13} = x^{12} \cdot x \quad \text{und \( x^{12}\) weiterhin als} \quad x^{12} = (x^{6})^2.
\end{align*}

Ausgehend von dieser Beobachtung, ergibt sich die folgende Rekursionsformel \cite{lang}:
...
Das modulare Inverse ist eine weitere wichtige Methode der Kryptographie. Wenn a eine ganze Zahl $a \in \mathbf{Z_p} $ und p eine Primzahl ist, ist das
...
Das modulare Inverse wird mit Hilfe des extended Euklidean Algorithmus (EEA) bestimmt.
...
Die ganze Idee ist, mit der ggT zu beginnen und rekursiv rückwärts zu arbeiten. Dies kann erreicht werden, indem die Zahlen als Variablen behandelt werden, bis der Ausdruck \(1 = x \cdot a + y \cdot p \) erhalten wird, der eine lineare Kombination unserer Anfangszahlen ist. \\
Daraus es folgt \( x \cdot a \equiv 1 \mod p \). 
Mit Hilfe des modularen multiplikativen Inversen lässt sich leicht die modulare Division zwischen 2 Zahlen bestimmen.
